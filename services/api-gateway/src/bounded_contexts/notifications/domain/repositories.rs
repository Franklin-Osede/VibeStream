...
pub trait NotificationRepository: Send + Sync {
    ...
    async fn search(&self, filters: &NotificationFilters, page: u32, page_size: u32) -> Result<Vec<Notification>, Box<dyn std::error::Error + Send + Sync>>;
...
pub trait NotificationPreferencesRepository: Send + Sync {
    async fn get_by_user_id(&self, user_id: Uuid) -> Result<Option<NotificationPreferences>, Box<dyn std::error::Error + Send + Sync>>;
    async fn create(&self, preferences: &NotificationPreferences) -> Result<(), Box<dyn std::error::Error + Send + Sync>>;
    async fn update(&self, preferences: &NotificationPreferences) -> Result<(), Box<dyn std::error::Error + Send + Sync>>;
    async fn delete(&self, user_id: Uuid) -> Result<(), Box<dyn std::error::Error + Send + Sync>>;
}

#[async_trait]
pub trait NotificationTemplateRepository: Send + Sync {
    async fn create(&self, template: &NotificationTemplate) -> Result<(), Box<dyn std::error::Error + Send + Sync>>;
    async fn get_by_id(&self, id: Uuid) -> Result<Option<NotificationTemplate>, Box<dyn std::error::Error + Send + Sync>>;
    async fn get_by_name(&self, name: &str) -> Result<Option<NotificationTemplate>, Box<dyn std::error::Error + Send + Sync>>;
    async fn get_all_active(&self) -> Result<Vec<NotificationTemplate>, Box<dyn std::error::Error + Send + Sync>>;
    async fn update(&self, template: &NotificationTemplate) -> Result<(), Box<dyn std::error::Error + Send + Sync>>;
    async fn delete(&self, id: Uuid) -> Result<(), Box<dyn std::error::Error + Send + Sync>>;
} 