...
impl EthereumChain {
    pub async fn new(private_key: &str, config: ChainConfig) -> Result<Self> {
        info!("Creating new EthereumChain instance...");
        ...
        debug!("Ethereum client initialized with config: {:?}", config);
...
impl Chain for EthereumChain {
    ...
    async fn get_balance(&self) -> Result<u64> {
        ...
        info!("Balance retrieved: {} ETH", self.from_base_unit(balance_u64));
        Ok(balance_u64)
    }
...
mod tests {
    ...
    async fn test_chain_creation() {
        ...
        let chain = EthereumChain::new(private_key, create_test_config())
            .await
            .unwrap();
